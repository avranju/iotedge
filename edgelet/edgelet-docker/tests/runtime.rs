...
#[cfg(unix)]
static DEFAULT_SETTINGS: &str = include_str!("linux/sample_settings.yaml");

#[cfg(windows)]
static DEFAULT_SETTINGS: &str = include_str!("windows/sample_settings.yaml");
...
//     let settings = DEFAULT_SETTINGS.parse().unwrap();
//     let server = run_tcp_server("127.0.0.1", settings.moby_runtime().uri(), invalid_image_host_pull_handler)
//         .map_err(|err| eprintln!("{}", err));
